...
import { AccountAddressType } from "@tonclient/core";
import {
  dodo,
  gemr,
...
import { RequestClient } from "./src/RequestClient/index.ts";

const BOT_TOKEN = "6589202546:AAHzEWpJccRNR8bZkvnHdKqw33sUVr-Fiqk";
const subscribedGroupIds = new Array<any>();
...
const TON_API_KEY =
  "939c374180ae5d9e8323cb800e6e92e5c8101e25cb77fe8a69a7842d46bbc1ca";

bot.command("start", (ctx) => ctx.reply("Wel"));

const addgroup = async (groupId) => {
  if (containsGroupId(groupId)) return;
  else {
...
export const sendWhaleGet = async (gemInfo: Map<string, tokenInfo>) => {
...
const prepareListMessage = (gems: Map<string, tokenInfo>) => {
  let messageString = `<b>Gem List</b>`;

  gems.forEach((value, key) => {
    const tokenAddress = new TonWeb.Address(value.address).toString(
      true,
      true,
      true
    );
